...
> ⚠️ **Don't use `uvx repotoire`** — it runs each command in a temporary environment that doesn't preserve state between commands. Use `uv pip install repotoire` or `uv tool install repotoire` instead.
