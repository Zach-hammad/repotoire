...
class CloudProxyClient(DatabaseClient):
    ...
    def ingest_git_commits(
        ...
        """Ingest git commit history into the temporal knowledge graph.

        This sends commit data extracted from the local git repository to the
        server-side Graphiti endpoint for processing.

        Args:
            repo_id: Repository UUID for multi-tenant isolation
            repo_slug: Repository slug (e.g., 'owner/repo')
            commits: List of commit data dicts with keys:
                - sha: Full commit SHA
                - author_name: Author name
                - author_email: Author email
                - committed_date: ISO format timestamp
                - message: Full commit message
                - changed_files: List of changed file paths
                - insertions: Lines inserted
                - deletions: Lines deleted
                - code_changes: List of function/class changes detected
            branch: Git branch name

        Returns:
            Statistics about the ingestion process
        """
        # Use a different client for the historical endpoint
        import httpx

        historical_url = f"{self.api_url}/api/v1/historical/ingest-commits"
        ...
        response = httpx.post(
            ...
            timeout=300.0,  # Git history can take a while
        )
