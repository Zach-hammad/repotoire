...
class SandboxUsageTracker:
    ...
    - Tracking concurrent sandbox sessions
    - Providing daily/monthly usage summaries
    - Aggregating usage from sandbox_usage table

    The tracker uses TimescaleDB for persistent storage and maintains
    an in-memory cache of concurrent sessions for fast lookups.
    ...
    def __init__(self, connection_string: Optional[str] = None):
        ...
        # In-memory concurrent session tracking for fast lookups
    ...
    async def get_concurrent_count(self, customer_id: str) -> int:
        ...
        Uses in-memory tracking for fast lookups.
    ...
    async def increment_concurrent(
        ...
        logger.debug(
            f"Incremented concurrent for {customer_id}: {count} active",
    ...
    async def decrement_concurrent(
        ...
        logger.debug(
            f"Decremented concurrent for {customer_id}: {count} active",
            extra={"sandbox_id": sandbox_id},
        )
