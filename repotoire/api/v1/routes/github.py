...
class RepositoryDetailResponse(BaseModel):
    """Response model for repository detail view (matches frontend Repository type)."""

    id: str = Field(..., description="Repository UUID as string")
    full_name: str = Field(..., description="Full repository name (owner/repo)")
    github_repo_id: int = Field(..., description="GitHub's repository ID")
    health_score: Optional[float] = Field(None, description="Latest health score (0-100)")
    last_analyzed_at: Optional[datetime] = Field(None, description="When last analyzed")
    analysis_status: str = Field("idle", description="Current analysis status")
    is_enabled: bool = Field(..., description="Whether analysis is enabled")
    default_branch: str = Field(..., description="Default branch name")
    created_at: datetime
    updated_at: datetime
...
async def get_repo_by_id(
    ...
    # Get repository with installation to verify org ownership
    result = await db.execute(
        select(GitHubRepository)
    ...
    # Get latest analysis run for health score and status
    # Use the linked Repository ID (from repositories table) for analysis data
    latest_analysis = None
    if repo.repository_id:
        analysis_result = await db.execute(
            select(AnalysisRun)
            .where(AnalysisRun.repository_id == repo.repository_id)
    ...
    if latest_analysis:
        analysis_status = latest_analysis.status.value if latest_analysis.status else "idle"
...
@router.post("/repos/{repo_id}/analyze", response_model=AnalyzeRepoResponse)
async def analyze_repo_by_id(
    repo_id: UUID,
    user: ClerkUser = Depends(require_org),
    db: AsyncSession = Depends(get_db),
    github: GitHubAppClient = Depends(get_github_client),
    ...
    """Trigger analysis for a repository by its UUID.
    ...
    # Get repository with installation
    result = await db.execute(
        select(GitHubRepository)
        .join(GitHubInstallation)
        .where(
    ...
    if not repo.enabled:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Repository is not enabled for analysis",
        )

    # Ensure we have a linked Repository record for analysis data
    ...
    if not repo.repository_id:
        ...
        if existing_repo:
            ...
            db.add(new_repo)
            await db.flush()
            repo.repository_id = new_repo.id
...
async def get_repo_analysis_status(
    ...
    # Get latest analysis run using linked Repository ID
    latest_analysis = None
    if repo.repository_id:
        analysis_result = await db.execute(
            select(AnalysisRun)
    ...
    return {
        ...
        "status": latest_analysis.status.value if latest_analysis.status else "unknown",
        "health_score": latest_analysis.health_score,
