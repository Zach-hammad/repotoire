...
CREATE INDEX IF NOT EXISTS idx_repo_time
    ON code_health_metrics (repository, time DESC);

CREATE INDEX IF NOT EXISTS idx_branch
    ON code_health_metrics (repository, branch, time DESC);
...
-- Continuous aggregate: daily health summary
...
GROUP BY day, repository, branch;
...
-- Continuous aggregate: weekly trend analysis
...
SELECT
    ...
    COUNT(*) AS num_analyses
FROM code_health_metrics
GROUP BY week, repository, branch;
...
-- View: Latest metrics for each repository/branch
CREATE OR REPLACE VIEW latest_metrics AS
SELECT DISTINCT ON (repository, branch)
    *
FROM code_health_metrics
ORDER BY repository, branch, time DESC;

-- View: Regression candidates (health score dropped >5 points)
CREATE OR REPLACE VIEW potential_regressions AS
WITH scored AS (
    SELECT
        *,
        LAG(overall_health) OVER (
            PARTITION BY repository, branch
            ORDER BY time
        ) AS prev_health
    FROM code_health_metrics
)
...
-- Function: Get trend direction (improving/declining/stable)
