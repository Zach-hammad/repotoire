...
Repotoire is a pure Rust CLI that runs all of this locally. No cloud service, no Docker, no external database. It uses petgraph for the in-memory graph and tree-sitter for parsing across 9 graph-native languages (Python, TypeScript, JavaScript, Rust, Go, Java, C#, C, C++).

110 detectors run in parallel via rayon. 77 default detectors cover security, code quality, and architecture. 33 additional deep-scan detectors cover code smells, style issues, and dead code.
