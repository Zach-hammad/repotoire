...
import { SANDBOX_REFRESH_INTERVAL_MS } from '@/lib/constants';
...
const warningLevelConfig: Record<SandboxWarningLevel, { color: string; bgColor: string; label: string }> = {
  ok: { color: 'text-success', bgColor: 'bg-success-muted', label: 'Healthy' },
  warning: { color: 'text-warning', bgColor: 'bg-warning-muted', label: 'Approaching Limit' },
  critical: { color: 'text-warning', bgColor: 'bg-warning-muted', label: 'Near Limit' },
  exceeded: { color: 'text-error', bgColor: 'bg-error-muted', label: 'Limit Exceeded' },
};
...
export default function SandboxDashboardPage() {
  ...
  const { data: quotaStatus, isLoading: quotaLoading, error: quotaError } = useSWR<SandboxQuotaStatus>(
    'sandbox-quota',
    () => sandboxApi.getQuota(),
    { refreshInterval: SANDBOX_REFRESH_INTERVAL_MS }
  );
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Card className="bg-info-muted border-info-semantic/20">
        <CardContent className="pt-6">
          <div className="flex items-start gap-4">
            <div className="p-3 rounded-full bg-info-muted">
              <Shield className="h-6 w-6 text-info-semantic" />
            </div>
            <div>
      ...
      {hasError && (
        <Card className="border-error bg-error-muted">
          <CardContent className="pt-6">
                        ...
                        <TableRow key={op.operation_type}>
                          ...
                          <TableCell className="text-right">
                            <Badge
                              variant="outline"
                              className={cn(
                                op.success_rate >= 95 && 'border-success text-success',
                                op.success_rate >= 80 && op.success_rate < 95 && 'border-warning text-warning',
                                op.success_rate < 80 && 'border-error text-error'
                              )}
                            >
            ...
            <CardContent>
              <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <div className="p-3 rounded-full bg-info-muted">
                      <Cpu className="h-6 w-6 text-info-semantic" />
                    </div>
                    <div>
                    ...
                    {usageStats.recent_failures.map((failure) => {
                      ...
                      return (
                        <TableRow key={failure.operation_id}>
                          ...
                          <TableCell className="max-w-xs truncate text-error">
          ...
          <Card>
            ...
            <CardContent>
              {usageStats?.slow_operations && usageStats.slow_operations.length > 0 ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  <Zap className="h-12 w-12 mx-auto mb-4 text-success opacity-50" />
    ...
    <Card className={cn(config.bgColor)}>
      ...
      <CardContent>
        ...
        <Progress
          ...
          className={cn(
            'mt-2 h-2',
            usage.warning_level === 'exceeded' && '[&>div]:bg-error',
            usage.warning_level === 'critical' && '[&>div]:bg-warning',
            usage.warning_level === 'warning' && '[&>div]:bg-warning',
          )}
        />
