...
export default function MonorepoPage() {
  ...
  const columns: DataTableColumn<PackageInfo>[] = [
    ...
    },
    {
      ...
      header: 'Type',
      cell: (pkg) => {
        const colorClass = packageTypeColors[pkg.metadata.package_type] || 'bg-gray-500/10 text-gray-500 border-gray-500/20';
        return (
    ...
    },
    {
      ...
      cell: (pkg) => (
        <div className="flex items-center gap-2">
          <TestTube className={cn('h-4 w-4', pkg.has_tests ? 'text-green-500' : 'text-muted-foreground')} />
                ...
                <CardContent>
                  <div className="space-y-1">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className={healthResult.cross_package_issues > 0 ? 'text-orange-500' : ''}>
                        {healthResult.cross_package_issues}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Circular deps</span>
                      <span className={healthResult.circular_dependencies > 0 ? 'text-red-500' : ''}>
