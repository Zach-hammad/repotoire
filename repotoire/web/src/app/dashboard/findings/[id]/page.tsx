...
// Status badge colors - using semantic colors where applicable
const statusBadgeVariants: Record<FindingStatus, string> = {
  open: 'bg-muted text-muted-foreground',
  acknowledged: 'bg-info-muted text-info-semantic',
  in_progress: 'bg-muted text-foreground',
  resolved: 'bg-success-muted text-success',
  wontfix: 'bg-warning-muted text-warning',
...
export default function FindingDetailPage({ params }: FindingDetailPageProps) {
  ...
  return (
    <TooltipProvider>
      ...
      <Card className={cn('border-l-4', sevConfig.borderColor)} size="compact">
        <CardContent>
      ...
      <Card className="bg-info-muted border-info-semantic/20">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg flex items-center gap-2">
            <HelpCircle className="h-5 w-5 text-info-semantic" />
            What should I do?
          </CardTitle>
        </CardHeader>
          ...
          {finding.severity === 'critical' || finding.severity === 'high' ? (
            <div className="space-y-2">
              <div className="flex items-start gap-2">
                <ChevronRight className="h-4 w-4 mt-0.5 text-info-semantic shrink-0" />
                <p className="text-sm">
