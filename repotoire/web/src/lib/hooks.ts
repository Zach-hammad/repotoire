...
export function useRepositories(params?: { page?: number; pageSize?: number }) {
  const { isAuthReady } = useApiAuth();
  return useSWR<RepositoryInfo[]>(
    isAuthReady ? ['repositories', params?.page, params?.pageSize] : null,
    () => repositoriesApi.list(params)
  );
}
 ...
 */
export function useRepositoriesFull(params?: { page?: number; pageSize?: number }) {
  const { isAuthReady } = useApiAuth();
  return useSWR<RepositoryInfo[]>(
    isAuthReady ? ['repositories-full', params?.page, params?.pageSize] : null,
    () => repositoriesApi.list(params)
  );
}
...
export function useCommit(repositoryId: string | null, commitSha: string | null) {
  ...
  return useSWR(
    ...
    {
      revalidateOnFocus: false,
      dedupingInterval: CACHE_DEDUP_INTERVAL_MS, // commit data is immutable
    }
  );
}
