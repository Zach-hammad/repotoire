...
export const repositoriesApi = {
  list: async (params?: { page?: number; pageSize?: number }): Promise<RepositoryInfo[]> => {
    const searchParams = new URLSearchParams();
    if (params?.page) searchParams.set('page', String(params.page));
    if (params?.pageSize) searchParams.set('page_size', String(params.pageSize));
    const query = searchParams.toString();
    return request<RepositoryInfo[]>(`/analytics/repositories${query ? `?${query}` : ''}`);
  },
};
...
export const narrativesApi = {
  ...
  streamSummary: (repositoryId: string, onChunk: (text: string) => void, onError?: (error: Error) => void): EventSource => {
    // Use Next.js API route proxy for auth (EventSource doesn't support headers)
    const url = `/api/narratives/stream?repository_id=${repositoryId}`;
