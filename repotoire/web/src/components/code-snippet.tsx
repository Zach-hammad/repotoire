            ...
            {lines.map((line, index) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    'flex',
                    isHighlighted && 'bg-yellow-500/20 -mx-4 px-4 border-l-2 border-yellow-500'
                  )}
                >
                  {showLineNumbers && (
                    <span
                      className={cn(
                        ...
                        isHighlighted && 'text-yellow-500 font-medium'
                      )}
                    >
                      {lineNumber}
                    </span>
    ...
    <div className="rounded-lg border overflow-hidden">
      ...
      {view === 'split' ? (
        <div className="grid grid-cols-2 divide-x">
          {/* Original */}
          <div>
            <div className="px-3 py-1.5 bg-red-500/10 border-b text-xs font-medium text-red-400">
              Original
            </div>
              ...
              <code className={getLanguageClass(language)}>
                {originalLines.map((line, index) => (
                  <div key={index} className="flex">
                    ...
                    <span className="whitespace-pre text-red-400/80">{line || ' '}</span>
                  </div>
                ))}
              </code>
            </pre>
          ...
          {/* Fixed */}
          <div>
            <div className="px-3 py-1.5 bg-green-500/10 border-b text-xs font-medium text-green-400">
              Fixed
            </div>
            <pre className="p-3 text-sm overflow-auto max-h-[300px]">
              <code className={getLanguageClass(language)}>
                {fixedLines.map((line, index) => (
                  <div key={index} className="flex">
                    ...
                    <span className="whitespace-pre text-green-400/80">{line || ' '}</span>
                  </div>
                ))}
              </code>
            </pre>
        ...
        <pre className="p-3 text-sm overflow-auto max-h-[400px]">
          <code className={getLanguageClass(language)}>
            {/* Show removed lines */}
            {originalLines.map((line, index) => (
              <div key={`old-${index}`} className="flex bg-red-500/10">
                <span className="select-none pr-2 text-red-400 min-w-[1rem]">-</span>
                <span className="select-none pr-3 text-muted-foreground text-right min-w-[2.5rem]">
                  {startLine + index}
                </span>
                <span className="whitespace-pre text-red-400/80">{line || ' '}</span>
            ...
            {/* Show added lines */}
            {fixedLines.map((line, index) => (
              <div key={`new-${index}`} className="flex bg-green-500/10">
                <span className="select-none pr-2 text-green-400 min-w-[1rem]">+</span>
                <span className="select-none pr-3 text-muted-foreground text-right min-w-[2.5rem]">
                  {startLine + index}
                </span>
                <span className="whitespace-pre text-green-400/80">{line || ' '}</span>
