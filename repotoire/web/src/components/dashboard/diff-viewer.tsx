            ...
            {diff.map((line, index) => (
              <tr
                ...
                className={cn(
                  ...
                  line.type === 'removed' && 'bg-error-muted'
                )}
              >
  ...
  return (
    <div className={cn('rounded-lg border overflow-hidden', className)}>
      ...
      <div className="grid grid-cols-2 border-b">
        <div className="border-r bg-error-muted px-4 py-2">
          <span className="font-mono text-sm text-error">Original</span>
        </div>
        <div className="bg-success-muted px-4 py-2">
          <span className="font-mono text-sm text-success">Fixed</span>
        </div>
      </div>
          ...
          <table className="w-full font-mono text-sm">
            <tbody>
              {originalLines.map((line, index) => (
                <tr key={index} className="border-b last:border-0 bg-error-muted/50">
                  <td className="w-12 select-none border-r px-2 py-0.5 text-right text-muted-foreground">
          ...
          <table className="w-full font-mono text-sm">
            <tbody>
              {fixedLines.map((line, index) => (
                <tr key={index} className="border-b last:border-0 bg-success-muted/50">
                  <td className="w-12 select-none border-r px-2 py-0.5 text-right text-muted-foreground">
                    {change.start_line + index}
                  </td>
                  <td className="whitespace-pre px-2 py-0.5">{line || ' '}</td>
