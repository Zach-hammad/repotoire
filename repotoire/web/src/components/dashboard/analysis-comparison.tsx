...
export function AnalysisComparison({
  ...
  const overallTrend = useMemo(() => {
    if (scoreDiff > 5) return { label: 'Significant Improvement', icon: TrendingUp, color: 'text-success' };
    if (scoreDiff > 0) return { label: 'Improvement', icon: TrendingUp, color: 'text-success' };
    if (scoreDiff < -5) return { label: 'Significant Decline', icon: TrendingDown, color: 'text-error' };
    if (scoreDiff < 0) return { label: 'Decline', icon: TrendingDown, color: 'text-error' };
