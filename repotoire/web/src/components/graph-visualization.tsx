  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      nodesRef.current.forEach((node) => {
        ...
        // Use semantic colors - these map to CSS variables
        if (node.type === "issue") {
          ctx.fillStyle = "oklch(0.65 0.25 25)" // error red
        } else if (node.type === "service") {
          ctx.fillStyle = "oklch(0.75 0.15 200)" // accent cyan
        } else if (node.type === "component") {
          ctx.fillStyle = "oklch(0.55 0.25 295)" // primary purple
        } else {
          ctx.fillStyle = "oklch(0.55 0.02 260)" // muted
        }
        ctx.fill()
    ...
    <div className="relative w-full h-full bg-gradient-to-br from-background via-muted/20 to-background" role="img" aria-label="Interactive code dependency graph visualization showing 12 modules including services, components, and utilities. A circular dependency issue has been detected between AuthService, UserController, OrderService, DatabaseUtil, CacheManager, and back to AuthService.">
      ...
      <div className="absolute bottom-4 left-4 flex flex-wrap gap-4 text-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-muted-foreground">Issue Detected</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-8 h-0.5 bg-error" style={{ borderStyle: "dashed" }} />
          <span className="text-muted-foreground">Circular Dependency</span>
        </div>
      </div>

      {/* Issue callout */}
      <div className="absolute top-4 right-4 bg-error/10 border border-error/30 rounded-lg px-4 py-3 max-w-xs">
