...
# Note: graph_client fixture is provided by tests/integration/conftest.py
...
class TestIncrementalIngestion:
    ...
    def test_new_file_detection(self, temp_repo, graph_client):
        ...
        # First ingestion
        pipeline = IngestionPipeline(str(temp_repo), graph_client)
        pipeline.ingest(incremental=True)

        stats1 = graph_client.get_stats()
        ...
        stats2 = graph_client.get_stats()
        assert stats2["total_files"] == 2

    def test_changed_file_detection(self, temp_repo, graph_client):
        ...
        rel_path = "module1.py"
        metadata1 = graph_client.get_file_metadata(rel_path)
        ...
        # Verify hash changed
        metadata2 = graph_client.get_file_metadata(rel_path)
        assert metadata2 is not None
        hash2 = metadata2["hash"]
        assert hash2 != hash1

    def test_unchanged_file_skipping(self, temp_repo, graph_client):
    ...
    def test_deleted_file_cleanup(self, temp_repo, graph_client):
        ...
        metadata = graph_client.get_file_metadata("module2.py")
        assert metadata is None

    def test_force_full_reingest(self, temp_repo, graph_client):
    ...
    def test_mixed_scenario(self, temp_repo, graph_client):
        ...
        assert graph_client.get_file_metadata("unchanged.py") is not None
        assert graph_client.get_file_metadata("to_change.py") is not None
        assert graph_client.get_file_metadata("new_file.py") is not None
        assert graph_client.get_file_metadata("to_delete.py") is None

    def test_incremental_performance_benefit(self, temp_repo, graph_client):
    ...
    def test_hash_computation_consistency(self, temp_repo, graph_client):
        ...
        metadata = graph_client.get_file_metadata("module1.py")
        assert metadata["hash"] == expected_hash

    def test_last_modified_tracking(self, temp_repo, graph_client):
    ...
    def test_entities_updated_on_change(self, temp_repo, graph_client):
