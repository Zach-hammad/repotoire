...
"#;
        // Use shorter paths that result in predictable module namespaces
        let files = vec![
            ("repotoire/graph/client.py".to_string(), client_source.to_string()),
        ...
        // file_to_module_ns converts repotoire/api/routes.py -> repotoire.api.routes
        let imports = ti.get_resolved_imports("repotoire.api.routes")
            .expect("Should have imports for repotoire.api.routes");
        let graph_client = imports.get("GraphClient")
            .expect("Should have GraphClient import (aliased from Neo4jClient)");
