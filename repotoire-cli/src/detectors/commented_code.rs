...
impl CommentedCodeDetector {
    ...
    fn is_annotation_comment(line: &str) -> bool {
        ...
        upper.contains("HACK") || upper.contains("NOTE:") || upper.contains("BUG:") ||
        ...
        for line in lines.get(start..end).unwrap_or(&[]) {
            for cap in func_ref().captures_iter(line) {
                if let Some(m) = cap.get(1) {
                    ...
                    if !["if", "for", "while", "function", "def", "class", "return", "import", "from"].contains(&name) {
    ...
    fn check_func_existence(graph: &GraphStore, refs: &HashSet<String>) -> (usize, usize) {
        let all_func_names: HashSet<String> = graph.get_functions()
            .into_iter()
            .map(|f| f.name)
            .collect();
...
impl Detector for CommentedCodeDetector {
    ...
    fn detect(&self, graph: &GraphStore) -> Result<Vec<Finding>> {
                ...
                while i < lines.len() {
                    ...
                    if is_comment && Self::looks_like_code(line) {
                        ...
                        if code_lines >= self.min_lines {
                            ...
                            if has_annotation {
                                notes.push("📝 Contains TODO/FIXME - may be intentionally preserved".to_string());
                            }
        ...
        info!("CommentedCodeDetector found {} findings (graph-aware)", findings.len());
        Ok(findings)
    }
}
