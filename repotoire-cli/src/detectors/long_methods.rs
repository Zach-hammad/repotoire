...
impl LongMethodsDetector {
    ...
    fn is_orchestrator(&self, graph: &GraphStore, qualified_name: &str, lines: u32, complexity: i64) -> bool {
...
impl Detector for LongMethodsDetector {
    ...
    fn detect(&self, graph: &GraphStore) -> Result<Vec<Finding>> {
        ...
        for func in graph.get_functions() {
            ...
            let is_orchestrator = self.is_orchestrator(graph, &func.qualified_name, lines, complexity);
            ...
            if is_orchestrator {
                notes.push(format!("📤 Orchestrator pattern: calls {} functions (reduced severity)", out_degree));
            }
            
            if density > 0.3 {
                notes.push(format!("⚠️ High complexity density: {:.2} (complexity {} / {} lines)", 
                                  density, complexity, lines));
            }
            
            if callee_clusters.len() >= 3 {
                notes.push(format!("🔀 Calls {} different modules - possible split points", callee_clusters.len()));
            }
            ...
            let suggestion = if is_orchestrator {
                "This appears to be an orchestrator function (coordinates many calls).\n\
                 ...
                 3. Has a clear flow (consider a state machine for complex flows)".to_string()
            } else if callee_clusters.len() >= 3 {
                format!(
                    "This function calls {} different modules. Consider extracting:\n{}",
                    callee_clusters.len(),
                    callee_clusters.iter().take(3)
                ...
                "High complexity density - this function does too much logic.\n\
                 ...
                 3. Consider the Strategy pattern for varying behaviors".to_string()
            } else {
            ...
            findings.push(Finding {
                ...
                estimated_effort: Some(if lines > 200 { "1-2 hours".to_string() } else { "30 minutes".to_string() }),
                category: Some("maintainability".to_string()),
                cwe_id: None,
                why_it_matters: Some(
                    "Long methods are hard to understand, test, and maintain. \
                     Each function should do one thing well.".to_string()
                ),
                ..Default::default()
            });
        }
        
        info!("LongMethodsDetector found {} findings (graph-aware)", findings.len());
        Ok(findings)
    }
}
