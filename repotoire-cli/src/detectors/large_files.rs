...
impl LargeFilesDetector {
    ...
    fn analyze_file_structure(graph: &GraphStore, file_path: &str) -> FileAnalysis {
        let functions: Vec<_> = graph.get_functions()
            .into_iter()
        ...
        let largest_func = functions.iter()
    ...
    fn detect(&self, graph: &GraphStore) -> Result<Vec<Finding>> {
        ...
        for entry in walker.filter_map(|e| e.ok()) {
            ...
            if path_str.contains("vendor") || path_str.contains("node_modules") ||
               path_str.contains("generated") || path_str.contains(".min.") {
                continue;
            }
            ...
            if let Some(content) = crate::cache::global_cache().get_content(path) {
                ...
                if lines > self.threshold {
                    ...
                    if analysis.importer_count > 0 {
                        notes.push(format!("🔗 {} files depend on this", analysis.importer_count));
                    }
                    ...
                    let suggestion = if !analysis.potential_modules.is_empty() {
                        format!(
                            "Consider splitting by function prefix into separate modules:\n\n\
                             ...
                             ```",
                            analysis.potential_modules.iter()
                                .map(|p| format!("• `{}_*` functions → `{}.py`", p, p))
                                .collect::<Vec<_>>()
                                .join("\n"),
                            analysis.potential_modules.first().unwrap_or(&"module".to_string()),
                            analysis.potential_modules.first().unwrap_or(&"module".to_string()),
                            path.file_stem().and_then(|s| s.to_str()).unwrap_or("file")
                        )
                    } else {
                        "Split into smaller, focused modules. Group related functions together.".to_string()
                    };
                    ...
                    findings.push(Finding {
                        ...
                        why_it_matters: Some(if analysis.importer_count > 5 {
                            "This file is a dependency hub - many other files import from it. \
                             Large dependency hubs are hard to refactor and create merge conflicts.".to_string()
                        } else {
                            "Large files are harder to understand, test, and maintain. \
                             They often indicate that the module has too many responsibilities.".to_string()
                        }),
                        ..Default::default()
                    });
                }
        ...
        info!("LargeFilesDetector found {} findings (graph-aware)", findings.len());
        Ok(findings)
    }
}
