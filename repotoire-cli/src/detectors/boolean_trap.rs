...
impl BooleanTrapDetector {
    ...
    fn extract_func_name(line: &str) -> Option<String> {
        func_call().captures(line).and_then(|c| c.get(1).map(|m| m.as_str().to_string()))
    }
...
impl Detector for BooleanTrapDetector {
    ...
    fn detect(&self, graph: &GraphStore) -> Result<Vec<Finding>> {
                    ...
                    if bool_args().is_match(line) {
                        if let Some(func_name) = Self::extract_func_name(line) {
                            ...
                            *func_call_counts.entry(func_name.clone()).or_default() += 1;
                            trap_calls.push((path.to_path_buf(), (i + 1) as u32, func_name, bool_count));
                        }
                    }
                }
            }
        ...
        for (path, line_num, func_name, bool_count) in trap_calls {
            ...
            let func_def = graph.get_functions()
                .into_iter()
            ...
            if bool_count > 2 {
                notes.push(format!("⚠️ {} boolean arguments (very confusing)", bool_count));
            }
            ...
            if let Some(ref def) = func_def {
                ...
                if callers.len() > 5 {
                    notes.push(format!("🔥 Widely used ({} callers) - high impact fix", callers.len()));
                }
            }
            ...
            findings.push(Finding {
                ...
                estimated_effort: Some(if call_count > 5 { "30 minutes".to_string() } else { "15 minutes".to_string() }),
                category: Some("readability".to_string()),
                cwe_id: None,
                why_it_matters: Some(
                    "Boolean traps make APIs confusing and error-prone. \
                     It's easy to swap arguments or forget their meaning.".to_string()
                ),
                ..Default::default()
            });
        }
        
        info!("BooleanTrapDetector found {} findings (graph-aware)", findings.len());
        Ok(findings)
    }
}
