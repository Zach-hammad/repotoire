...
impl DebugCodeDetector {
    ...
    fn is_logging_utility(func_name: &str) -> bool {
        let logging_patterns = ["log", "debug", "trace", "print", "dump", "inspect", "show", "display"];
    ...
    fn is_dev_only_path(path: &str) -> bool {
        let dev_patterns = ["/dev/", "/debug/", "/utils/debug", "/helpers/debug", 
                          "debug_", "_debug.", "/logging/"];
...
impl Detector for DebugCodeDetector {
    ...
    fn detect(&self, graph: &GraphStore) -> Result<Vec<Finding>> {
        ...
        for entry in walker.filter_map(|e| e.ok()) {
            ...
            if Self::is_dev_only_path(&path_str) { continue; }
            ...
            if let Some(content) = crate::cache::global_cache().get_content(path) {
                ...
                for (i, line) in content.lines().enumerate() {
                    ...
                    if debug_pattern().is_match(line) {
                        ...
                        // Calculate severity
                        let severity = if line.contains("pdb") || line.contains("debugger") || 
                        ...
                        if file_debug_count > 1 {
                            notes.push(format!("📊 {} debug statements in this file so far", file_debug_count));
                        }
                        ...
                        findings.push(Finding {
                            ...
                            why_it_matters: Some(
                                "Debug code can leak sensitive information, clutter logs, \
                                 and interactive debuggers will hang the application.".to_string()
                            ),
                            ..Default::default()
                        });
                    }
        ...
        info!("DebugCodeDetector found {} findings (graph-aware)", findings.len());
        Ok(findings)
    }
}
