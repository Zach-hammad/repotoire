...
impl Detector for DjangoSecurityDetector {
    ...
    fn detect(&self, graph: &GraphStore) -> Result<Vec<Finding>> {
            ...
            if let Some(content) = crate::cache::global_cache().get_content(path) {
                ...
                for (i, line) in lines.iter().enumerate() {
                    ...
                    if csrf_exempt().is_match(line) {
                        let func_context = Self::find_containing_function(graph, &path_str, line_num);
                        ...
                        } else if func_context.as_ref().map(|(_, _, is_view)| *is_view).unwrap_or(false) {
                        ...
                        if let Some((func_name, callers, is_view)) = &func_context {
                            notes.push(format!("📦 Function: `{}` ({} callers)", func_name, callers));
                            if *is_view {
                        ...
                        findings.push(Finding {
                            ...
                            why_it_matters: Some(
                                "CSRF exemption allows attackers to trick users into performing \
                                 unintended actions through malicious websites or links.".to_string()
                            ),
                            ..Default::default()
                        });
                    }
                    ...
                    if debug_true().is_match(line) {
                        if fname.contains("settings") && !fname.contains("dev") && !fname.contains("local") && !fname.contains("test") {
                            findings.push(Finding {
                                ...
                                why_it_matters: Some("Debug mode leaks sensitive information to attackers.".to_string()),
                                ..Default::default()
                            });
                        }
                    }
                    ...
                    if secret_key().is_match(line) && !line.contains("os.environ") && !line.contains("env(") {
                        if fname.contains("settings") && !fname.contains("dev") && !fname.contains("local") {
                            findings.push(Finding {
                                ...
                                why_it_matters: Some("Leaked SECRET_KEY allows session hijacking and data tampering.".to_string()),
                                ..Default::default()
                            });
                        }
                    }
                    ...
                    if raw_sql().is_match(line) {
                        ...
                        let mut notes = Vec::new();
                        if has_user_input {
                            notes.push("⚠️ String interpolation detected - possible SQL injection".to_string());
                        }
                        if let Some((func_name, callers, is_view)) = &func_context {
                        ...
                        findings.push(Finding {
                            ...
                            why_it_matters: Some("Raw SQL with user input can lead to SQL injection.".to_string()),
                            ..Default::default()
                        });
                    }
                }
        ...
        info!("DjangoSecurityDetector found {} findings (graph-aware)", findings.len());
        Ok(findings)
    }
}
