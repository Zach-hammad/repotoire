...
fn cookie_pattern() -> &'static Regex {
    COOKIE_PATTERN.get_or_init(|| {
        Regex::new(r"(?i)(set.cookie|cookie\s*=|res\.cookie|response\.set_cookie|setcookie|\.cookies\[)").unwrap()
    })
}
...
impl InsecureCookieDetector {
    ...
    fn check_cookie_flags(lines: &[&str], cookie_line: usize) -> CookieFlags {
        ...
        CookieFlags {
            ...
            samesite_value: if context.contains("samesite=strict") || context.contains("samesite='strict'") {
...
impl Detector for InsecureCookieDetector {
    ...
    fn detect(&self, graph: &GraphStore) -> Result<Vec<Finding>> {
        ...
        for entry in walker.filter_map(|e| e.ok()) {
            ...
            if !matches!(ext, "py"|"js"|"ts"|"php"|"rb"|"java"|"go") { continue; }
            ...
            if let Some(content) = crate::cache::global_cache().get_content(path) {
                ...
                for (i, line) in lines.iter().enumerate() {
                    if cookie_pattern().is_match(line) {
                        ...
                        let (is_sensitive, cookie_type) = Self::is_sensitive_cookie(line, &surrounding);
                        ...
                        if !flags.has_httponly { missing.push("HttpOnly"); }
                        if !flags.has_secure { missing.push("Secure"); }
                        if !flags.has_samesite { missing.push("SameSite"); }
        ...
        info!("InsecureCookieDetector found {} findings (graph-aware)", findings.len());
        Ok(findings)
    }
}
