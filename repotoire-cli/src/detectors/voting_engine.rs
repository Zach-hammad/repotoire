...
mod tests {
    ...
    #[test]
    fn test_single_finding_above_threshold() {
        let engine = VotingEngine::new();
    ...
    #[test]
    fn test_consensus_from_multiple_detectors() {
        let engine = VotingEngine::new();
        let findings = vec![
            create_test_finding("GodClassDetector", Severity::High, "test.py"),
            create_test_finding("RadonDetector", Severity::Medium, "test.py"),
        ];
        let (results, stats) = engine.vote(findings);
        // Should be grouped and merged
        assert_eq!(stats.groups_analyzed, 1);
        assert!(results[0].detector.contains("Consensus"));
    }

    #[test]
    fn test_severity_resolution_highest() {
        let engine = VotingEngine::new();
        let findings = vec![
            create_test_finding("Detector1", Severity::Low, "test.py"),
            create_test_finding("Detector2", Severity::Critical, "test.py"),
        ];
        let (results, _) = engine.vote(findings);
    ...
    fn test_different_categories_not_merged() {
        ...
        let findings = vec![
            create_test_finding("GodClassDetector", Severity::High, "test.py"),
            create_test_finding("BanditDetector", Severity::High, "test.py"),
        ];
        let (results, stats) = engine.vote(findings);
