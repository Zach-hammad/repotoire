    ...
    fn suggest_exceptions(calls: &HashSet<String>, ext: &str) -> Vec<String> {
        ...
        for call in calls {
            ...
            if file_ops.iter().any(|op| call_lower.contains(op)) {
                match ext {
                    "py" => suggestions.push("IOError, FileNotFoundError, PermissionError".to_string()),
                    "java" => suggestions.push("IOException, FileNotFoundException".to_string()),
                    "js" | "ts" => suggestions.push("Error (check error.code for ENOENT, EACCES)".to_string()),
            ...
            if network_ops.iter().any(|op| call_lower.contains(op)) {
                match ext {
                    "py" => suggestions.push("requests.RequestException, urllib.error.URLError, ConnectionError".to_string()),
                    "java" => suggestions.push("IOException, SocketException, HttpClientErrorException".to_string()),
                    "js" | "ts" => suggestions.push("TypeError (network errors), AbortError".to_string()),
            ...
            if parse_ops.iter().any(|op| call_lower.contains(op)) {
                match ext {
                    "py" => suggestions.push("json.JSONDecodeError, ValueError, UnicodeDecodeError".to_string()),
                    "java" => suggestions.push("JsonParseException, NumberFormatException".to_string()),
            ...
            if db_ops.iter().any(|op| call_lower.contains(op)) {
                match ext {
                    "py" => suggestions.push("sqlite3.Error, psycopg2.Error, pymysql.Error".to_string()),
...
impl Detector for BroadExceptionDetector {
    ...
    fn detect(&self, graph: &GraphStore) -> Result<Vec<Finding>> {
            ...
            if let Some(content) = crate::cache::global_cache().get_content(path) {
                ...
                for (i, line) in lines.iter().enumerate() {
                    if broad_except().is_match(line) {
                        ...
                        let next_lines = lines.get(i+1..i+4)
                            .map(|s| s.join(" "))
                            .unwrap_or_default();
        ...
        info!("BroadExceptionDetector found {} findings (graph-aware)", findings.len());
        Ok(findings)
    }
}
