...
5. **Consolidate into single function** with parameters for variations"#
    ...
    fn create_finding(&self, cluster: &BoilerplateCluster) -> Finding {
        ...
        // Build description
        let func_names: Vec<_> = cluster.functions.iter().map(|f| f.node_name(crate::graph::interner::global_interner()).to_string()).collect();
        ...
        let files: HashSet<_> = cluster.functions.iter().map(|f| f.path(crate::graph::interner::global_interner())).collect();
        ...
        let affected_files: Vec<PathBuf> = cluster
            .functions
            .iter()
            .map(|f| PathBuf::from(f.path(crate::graph::interner::global_interner())))
            .collect::<HashSet<_>>()
            .into_iter()
            .collect();
