...
fn express_app() -> &'static Regex {
    EXPRESS_APP.get_or_init(|| Regex::new(r#"express\(\)|require\(["']express["']\)|from ['"]express['"']"#).unwrap())
}
...
impl SecurityFeatures {
    fn from_content(content: &str) -> Self {
        ...
        Self {
            ...
            has_rate_limit: lower.contains("ratelimit") || lower.contains("rate-limit") || lower.contains("express-rate"),
            has_body_parser_limit: content.contains("limit:") || content.contains("bodyParser") && content.contains("limit"),
    ...
    fn count_auth_middleware(content: &str) -> usize {
        ...
        auth_patterns.iter().filter(|p| content.contains(*p)).count()
    }
    ...
    fn security_score(&self) -> f64 {
        ...
        // Helmet - critical
        max_score += 25.0;
        if self.has_helmet { score += 25.0; }

        // Rate limiting - critical for production
        max_score += 20.0;
        if self.has_rate_limit { score += 20.0; }

        // Body parser limits
        max_score += 15.0;
        if self.has_body_parser_limit { score += 15.0; }

        // CORS
        max_score += 10.0;
        if self.has_cors { score += 10.0; }

        // CSRF for non-API apps
        max_score += 10.0;
        if self.has_csrf { score += 10.0; }

        // Auth middleware
        max_score += 20.0;
        if self.auth_middleware_count > 0 { score += 20.0; }
...
impl Detector for ExpressSecurityDetector {
    ...
    fn detect(&self, _graph: &GraphStore) -> Result<Vec<Finding>> {
        ...
        for entry in walker.filter_map(|e| e.ok()) {
            ...
            if !matches!(ext, "js"|"ts"|"mjs") { continue; }
            ...
            if let Some(content) = crate::cache::global_cache().get_content(path) {
                ...
                if !features.has_helmet { missing.push("helmet"); }
                if !features.has_rate_limit { missing.push("rate-limit"); }
                if !features.has_body_parser_limit { missing.push("body-parser-limit"); }
                if features.auth_middleware_count == 0 { missing.push("auth-middleware"); }
                    ...
                    findings.push(Finding {
                        ...
                        why_it_matters: Some(
                            "Without rate limiting, attackers can:\n\
                             ...
                             • DoS your API\n\
                             • Abuse expensive endpoints".to_string()
                        ),
                        ..Default::default()
                    });
                }
                ...
                if !features.has_body_parser_limit {
                    findings.push(Finding {
                        ...
                        description: "Large request bodies can be used for DoS attacks.".to_string(),
                        affected_files: vec![path.to_path_buf()],
                        line_start: Some(1),
                        line_end: Some(1),
                        suggested_fix: Some(
                        ...
                        why_it_matters: Some("Large payloads can exhaust server memory.".to_string()),
                        ..Default::default()
                    });
                }
        ...
        info!("ExpressSecurityDetector found {} findings (graph-aware)", findings.len());
        Ok(findings)
    }
}
