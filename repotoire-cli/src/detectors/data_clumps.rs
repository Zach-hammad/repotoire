...
impl DataClumpsDetector {
    ...
    fn analyze_call_relationships_cached(
        ...
        let func_qns: HashSet<&str> = funcs.iter().map(|f| &f.qualified_name).collect();
        let mut call_count = 0;
        let mut has_chain = false;

        for func in funcs {
            if let Some(callees) = callees_map.get(&func.qualified_name) {
    ...
    fn remove_subsets(&self, mut clumps: Vec<DataClump>) -> Vec<DataClump> {
        ...
        for clump in clumps {
            ...
            let func_set: HashSet<&str> = clump.funcs.iter().map(|f| &f.name).collect();
            ...
            let is_subset = result.iter().any(|existing: &DataClump| {
                ...
                let existing_funcs: HashSet<&str> =
                    existing.funcs.iter().map(|f| &f.name).collect();
