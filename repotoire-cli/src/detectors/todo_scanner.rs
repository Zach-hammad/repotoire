...
impl TodoScanner {
    ...
    fn categorize_todo(msg: &str) -> (&'static str, Option<&'static str>) {
        ...
        if msg_lower.contains("security") || msg_lower.contains("auth") || msg_lower.contains("password") {
            return ("security", Some("⚠️ Security-related TODO"));
        }
        if msg_lower.contains("performance") || msg_lower.contains("slow") || msg_lower.contains("optimize") {
        ...
        if msg_lower.contains("refactor") || msg_lower.contains("cleanup") || msg_lower.contains("remove") {
...
impl Detector for TodoScanner {
    ...
    fn detect(&self, graph: &GraphStore) -> Result<Vec<Finding>> {
            ...
            if let Some(content) = crate::cache::global_cache().get_content(path) {
                ...
                for (line_num, line) in content.lines().enumerate() {
                    if let Some(caps) = get_pattern().captures(line) {
                        ...
                        let containing_func = Self::find_containing_function(graph, &path_str, line_u32);
                        ...
                        findings.push(Finding {
                            ...
                            description: format!(
                                ...
                                tag.to_uppercase(), context_notes
                            ),
                            ...
                            cwe_id: if category == "security" { Some("CWE-1078".to_string()) } else { None },
                            why_it_matters: Some(
                                "TODOs represent known issues or incomplete work. \
                                 Tracking and prioritizing them helps manage technical debt.".to_string()
                            ),
                            ..Default::default()
                        });
                    }
        ...
        info!("TodoScanner found {} findings (graph-aware)", findings.len());
        Ok(findings)
    }
}
